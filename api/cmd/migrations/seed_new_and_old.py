...
class SourceTableBuilder:
    ...
    async def seed_accommodation_logs(self):
        ...
        for row in self.raw_data["accommodation_logs_past"]:
            # if row["property_name"].strip() == "The Well Hotel":
            #     print(row)
            ...
            # Construct the property key, possibly ignoring the country ID
            if special_handling:
                property_key = (
                    row["property_name"].strip().lower(),
                    portfolio.id,  # Assuming you have portfolio_id in row
                    None,  # Ignoring country ID
                    core_destination.id,
                )
            else:
                property_key = (
