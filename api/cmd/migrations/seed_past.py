...
class SourceTableBuilder:
    ...
    async def seed_core_destinations(self):
        ...
        records_to_add = []
        for row in self.raw_data["core_destinations_past"]:
    ...
    async def seed_agencies(self):
        ...
        records_to_add = []
        for row in self.raw_data["agencies_past"]:
            # Add UUID and timestamps
            row["id"] = uuid.uuid4()
            row["created_at"] = datetime.now()
            row["updated_at"] = datetime.now()
    ...
    async def seed_booking_channels(self):
        ...
        records_to_add = []
        for row in self.raw_data["booking_channels_past"]:
            # Add UUID and timestamps
            row["id"] = uuid.uuid4()
            row["created_at"] = datetime.now()
            row["updated_at"] = datetime.now()
    ...
    async def seed_consultants(self):
        ...
        records_to_add = []
        for row in self.raw_data["consultants_past"]:
            # Add UUID and timestamps
            row["id"] = uuid.uuid4()
            row["created_at"] = datetime.now()
            row["updated_at"] = datetime.now()
    ...
    async def seed_portfolios(self):
        ...
        # records_to_add = []
        # properties = self.raw_data.get("properties_past", [])
        ...
        # # # Correct code
        records_to_add = []
        for row in self.raw_data["portfolios_past"]:
            # Add UUID and timestamps
            row["id"] = uuid.uuid4()
            row["created_at"] = datetime.now()
            row["updated_at"] = datetime.now()
    ...
    async def seed_countries(self):
        ...
        for row in self.raw_data["countries_past"]:
    ...
    async def seed_properties(self):
        ...
        # # code to initially crawl all properties to get list of countries for CSV
        # records_to_add = []
        # acc = self.raw_data.get("properties_past", [])
        ...
        for row in self.raw_data["properties_past"]:
    ...
    async def seed_accommodation_logs(self):
        ...
        consultant_map = {
            (cons.last_name.lower(), cons.first_name.lower()): cons
            for cons in all_consultants
        }
        agency_map = {agency.name: agency for agency in all_agencies}
        booking_channel_map = {bc.name: bc for bc in all_booking_channels}

        for row in self.raw_data["accommodation_logs_past"]:
            # fix 9/2/77 date_out
            if (
                row["primary_traveler"] == "McDowell/Whitney"
                and row["property_name"] == "Saxon Hotel Villas & Spa"
            ):
                row["date_out"] = "10/15/19"
            ...
            consultant_names = tuple(
                name.lower() for name in row["tb_consultant"].split("/")
            )
